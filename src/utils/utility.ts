...
type TModifiedAccountsList = NonNullable<
  ReturnType<typeof useAuthorize>["data"]["account_list"]
>[number] & {
  token?: string;
};
  ...
  account_list?.forEach((account) => {
    ...
    Object.keys(account).forEach((param) => {
      if (param === "loginid") {
        ...
        const value_to_set =
          typeof account[param as keyof typeof account] === "undefined" ? "" : account[param as keyof typeof account];
